...
При разработке приложения "Веб-ларек" использовался архитектурный подход MVP (Model, View, Presenter).  Взаимодействие компонентов приложения между собой происходит через события. Модели данных инициализируют события, которые в свою очередь осуществляют передачу данных компонентам отображения, также происходит изменение значения в моделях. 

### Класс ItemData 
Данный класс принимает 

### Слои приложения

#### 1. Слой Model
Данный слой используется для описания моделей данных, которые будут использоваться в проекте. 
